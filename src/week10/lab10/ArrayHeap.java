...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        while (inBounds(index) && inBounds(parentIndex(index))
                && contents[parentIndex(index)].priority() > contents[index].priority()) {
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);
        while (inBounds(index) && inBounds(2 * index)) {
            int newIndex = 2 * index;
            //找出优先级较小的那个
            if (newIndex < size && contents[newIndex].priority() > contents[newIndex + 1].priority()) {
                newIndex++;
            }
            //如果较小的那个都比当前的要大，那么已经sink结束
            if (!(contents[index].priority() > contents[newIndex].priority())) {
                break;
            }
            //否则交换当前节点和较小节点
            swap(index, newIndex);
